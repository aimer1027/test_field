...
/**
  ...
  pipe[0] ---- > is equal to the server's readin socket descriptor 
  		 and server's accept method return socket descriptor 

  pipe[1] ---- > is just like the client's write in socket descriptor
...
ret = 
setsockopt (pipes[1] , SOL_SOCKET , SO_RCVTIMEO , (const char *) &timeout ,sizeof(timeout) ); 
...
// here we use the write pipe connect to local loop address
...
/**
  method write contents into pipe 
  use pipes[1] as descriptor
...
**/
int  pipe::write_in_pipe ( const void *buf , int len )
{
...
int pipe::read_from_pipe ( void *buf , int len )
{
    return recv ( pipes[0] , (char *)buf , len , 0 ) ;
}

int pipe::get_read_descr ()
{
    return pipes[0] ; 
}
